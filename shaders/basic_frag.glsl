...
layout (location = 9) uniform vec3 sun_pos = vec3(0.0, 10.0, 0.0);
...
void main () {
  ...
  // vec3 view_dir = tbn_mat * normalize(view_pos - frag_position);
  // vec3 light_dir =  tbn_mat * normalize(light_pos - frag_position);
  ...
  vec3 diffuse = diffuse_scalar * sun.diffuse * texture(diffuse_texture, frag_uv).rgb * sun_intensity ;
  vec3 specular = specular_scalar * clamp( specular_scalar * sun.specular, 0, 1) * texture(specular_texture, frag_uv).rgb;
